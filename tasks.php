...
<td style="cursor:pointer;" onClick="window.open('bike-history.php?bike_id=<?php echo ($tasks_lst['bike']);?>', '_self');">
<?php $model_name=mysqli_query($db,"select (select mnf_name from mnf where id=(select mnf_id from models where id=(SELECT model FROM bike WHERE id='".$tasks_lst['bike']."'))) as make, modification, model, capacity from models where id=(SELECT model FROM bike WHERE id='".$tasks_lst['bike']."')");
$model=mysqli_fetch_array($model_name);
		    echo ($model['make']." ".$model['model'].$model['capacity'].$model['modification']); ?></td>
...
<td><?php
    ...
    ?>
</td><td style="cursor:pointer;" onClick="popupWin = window.open('edit/task.php<?php echo ("?id=".$tasks_lst['id']);?>', 'Редагувати задачу', 'location,width=800,height=680,top=0'); popupWin.focus(); return false;">
...
<td><a href='bill.php<?php echo ("?id=".$tasks_lst['id']);?>'><img src='images/pdf.png'></a></td>
<?php /*
<td align=center>
