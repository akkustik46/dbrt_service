...
(function(){
	...
	if (!window.def) {def = {}; addLog = function(){}; addNavig = function(){}; addArrows = function(c){c.title = "Sort"}; addEl = function(){}; addInputs = function(){}; }
	...
	prepTabs = function (t1){
		...
		for (tid in ts) {
			...
			for (k = 0; k < t.rlen; k ++) {
				...
				else {c.title = 'Sort';} //9:31 02.09.2011
				alt_always[k] = !!alt //12:06 04.09.2011
				t.col_color[k] = 0
			}
			
			irr = fixTime(null, 'makeThead', t)
			fixTime(null, 'makeSortArr', t, irr)
			t.page_size = (def.paginate) ? 30 : 0
			addNavig(t)
			fixTime(null, 'page', t)
			curr_t = t
			t.arrow_color = 0
...
//   t0 = ce("table")
		var tb = t.tBodies[0], val, /*ce("table").appendChild(*/
	...
	*/
	...
	var clicktab = function (e) {
		...
		var patch = def.patch_similar,
		 cmpRow = (!patch) //мухи отдельно ФФ отдельно
		  ? "(a == b) ? 0 : (a " + (verse ? "<" : ">") + " b) ? 1 : -1"
...
//alert(cmp)
		...
		if (patch) rebuildIdx(rarr) //"индекс" нужен для кривых не-gecko && like-gecko
		addLog("sort: ") //fixTime(rarr, 'sort', cmp)
		t.sorted = i

		fixTime(null, 'page', t)
		obj.title = "Sort by " + ((verse) ? "desc" : "asc")
	}
	...
	doFilter = function (inh) {
		...
		for (var i = 0; i < l; i++) { 
			...
			for (var j = 0; j < ql; j ++) {
				qq = query[j]
				if (!qq) continue
				if (!row.cells[j]) {continue} //такое возможно?!
