...
?>
<option value="<?php echo ($cl_lst['cl_id']."\""); 
...
<select name="bike_id" id="clbike" size="1" disabled="disabled">
...
$model_name=mysql_query("select (select mnf_name from mnf where id=(select mnf_id from models where id=(SELECT model FROM bike WHERE id='".$bike_lst['bike_id']."'))) as make, model, capacity from models where id=(SELECT model FROM bike WHERE id='".$bike_lst['bike_id']."')");
